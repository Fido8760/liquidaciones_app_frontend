...
import { deleteVarios } from "../../../../api/VariosAPI";
import { toast } from "react-toastify";
import Swal from "sweetalert2";
import { useLiquidacionPermissions } from "../../../../hooks/useLiquidacionPermissions";

type DetalleGastoViajeProps = {
liquidacion: Liquidacion;
}

export default function DetalleGastoViaje({ liquidacion }: DetalleGastoViajeProps) {
    ...
    const { mutate } = useMutation({
        ...
        queryClient.invalidateQueries({
            queryKey: ["liquidacion", liquidacion.id],
        });
        },
    });
            ...
            <nav className=" my-5 flex gap-3 justify-between items-center">
                ...
                {canEdit && (
                    <button
                        ...
                        onClick={() => navigate("?modalGasto=varios")}
                    >Agregar Gastos</button>
                )}
            </nav>
            <ul className="space-y-2">
                {liquidacion.gastos_varios.length > 0 ? (
                liquidacion.gastos_varios.map((gasto) => (
                    <li
                        key={gasto.id}
                        ...
                        <div className="flex-shrink-0 flex flex-row sm:flex-col gap-2 w-full sm:w-auto">
                            {canEdit && (
                                <>
                                    <button
                                        type="button"
                                        onClick={() => navigate( location.pathname + `?editar=varios&variosId=${gasto.id}` )}
                ...
                <p className="text-sm text-gray-500 dark:text-gray-300">
                    No hay gastos de combustible registrados.
                </p>
                )}
            </ul>
        </div>

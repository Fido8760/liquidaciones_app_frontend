...
export default function ModalAjustarLiquidacion({ liquidacion, onClose, }: ModalAjustarLiquidacionProps) {
  ...
  const { register, handleSubmit, watch, formState: { errors }, } = useForm<AjustarFormData>({ defaultValues: initialValues });
  ...
  const comisionMonto = liquidacion.total_bruto * (watchComision / 100);
  const netoEstimado = comisionMonto - totalAnticipos - watchAjuste;
              ...
              <Dialog.Panel className={ "w-full max-w-3xl transform overflow-hidden rounded-2xl bg-white dark:bg-gray-800 shadow-2xl transition-all" } >
                  ...
                  <div className=" flex items-center justify-between">
                    <div>
                      ...
                      <p className=" text-purple-100 text-sm mt-2">
                        ...
                        Operador:{" "}
                        <strong> {liquidacion.operador.nombre}{" "}{liquidacion.operador.apellido_p}{" "}{liquidacion.operador.apellido_m}</strong>
                      </p>
                    </div>
                    <button
                      onClick={onClose}
                ...
                <div className=" px-6 py-4 bg-blue-50 dark:bg-blue-900/20 border-b border-blue-200 dark:border-blue-800">
                  <div className=" grid grid-cols-3 gap-4 text-sm">
                    ...
                    <div>
                      ...
                      <p className=" font-bold text-blue-700 dark:text-blue-400">{formatCurrency(liquidacion.total_bruto)}</p>
                    </div>
                    <div>
                      <p className=" text-gray-600 dark:text-gray-400">Anticipos</p>
                      <p className=" font-bold text-blue-700 dark:text-blue-400">{formatCurrency(totalAnticipos)}</p>
                    </div>
                  </div>
                </div>
                {/** Formulario */}
                <form onSubmit={handleSubmit(handleForm)} className=" p-6" noValidate>
                  <div className=" space-y-6">
                    {/* Fieldset 1: Rendimiento y Comisión */}
                    <fieldset className="p-4 border dark:border-gray-700 rounded-lg">
                      <legend className="px-2 text-base font-semibold text-gray-800 dark:text-gray-200">Parámetros de Cálculo</legend>
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-5 mt-2">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
                            {watchComision}% de{" "}{formatCurrency(liquidacion.total_bruto)} ={" "} <strong className="text-purple-600 dark:text-purple-400">{formatCurrency(comisionMonto)}</strong>
                          </p>
                        </div>
                      </div>
                    </fieldset>
                    ...
                    <fieldset className=" p-4 border dark:border-gray-700 rounded-lg">
                      <legend className=" px-2 text-balance font-semibold text-gray-800 dark:text-gray-200">Ajustes Adicionales (Opcional)</legend>
                        ...
                        {requiereMotivo && (
                          <div>
                            ...
                            <textarea
                              ...
                              placeholder="Ej: Descuento por llanta ponchada, Bono por buen rendimiento..."
                    ...
                    <div className=" p-5 bg-gradient-to-br from-purple-50 to-indigo-50 dark:from-purple-900/20 dark:to-indigo-900/20 rounded-xl border-2 border-purple-200 dark:border-purple-700">
                      ...
                      <div className=" space-y-2 text-sm">
                        <div className=" flex justify-between">
                          ...
                          <span className=" font-semibold text-gray-900 dark:text-white">{formatCurrency(comisionMonto)}</span>
                        </div>
                        <div className=" flex justify-between">
                          <span className=" text-gray-700 dark:text-gray-300">Menos: Anticipos</span>
                          <span className="font-semibold text-red-600 dark:text-red-400" >-{formatCurrency(totalAnticipos)}</span>
                        </div>
                          {watchAjuste !== 0 && (
                            <div className=" flex justify-between">
                              <span className=" text-gray-700 dark:text-gray-300">Ajuste Manual</span>
                              <span  className={`font-semibold ${watchAjuste > 0 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`}>
                                {watchAjuste > 0 ? '-' : ''}{formatCurrency(watchAjuste)}
                              </span>
                            </div>
                          )}
                          <div className=" border-t-2 border-purple-300 dark:border-purple-600 pt-2 mt-2">
                            <div className=" flex justify-between items-center">
                              <span className=" font-bold text-purple-900 dark:text-purple-300">Total Neto a Pagar</span>
                              <span className={` text-2xl font-bold ${netoEstimado >= 0 ? ' text-purple-700 dark:text-purple-400' : ' text-red-700 dark:text-red-400'}`}>{formatCurrency(netoEstimado)}</span>
                            </div>
                          </div>
                          {netoEstimado < 0 && (
                            <div className="mt-2 p-3 bg-red-100 dark:bg-red-900/30 rounded-lg border border-red-300 dark:border-red-700">
                              <p className="text-xs text-red-800 dark:text-red-300 flex items-center gap-2">
                                ...
                                <strong>El operador deberá {formatCurrency(Math.abs(netoEstimado))} a la empresa</strong>
                              </p>
                            </div>
                          )}
                      </div>

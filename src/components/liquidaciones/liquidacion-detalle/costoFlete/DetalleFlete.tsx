...
type DetalleFleteProps = {
  liquidacion: Liquidacion;
};

export default function DetalleFlete({ liquidacion }: DetalleFleteProps) {
  ...
  const handleDeleteClick = (costoId: number) => {
    Swal.fire({
      title: '¿Estás seguro?',
    ...
    <div className="p-4">
      <nav className=" my-5 flex gap-3 justify-between items-center">
        <p className="text-sm font-semibold mb-4 dark:text-white">
          Detalle del costo de los fletes
        </p>
        {canEdit && (
          <button
            type="button"
            ...
            onClick={() => navigate("?modalGasto=flete")}
          >Agregar Flete</button>
        )}
      </nav>

...
import {
    createTipoGasto,
    getTipoGastos,
    toggleActivoTipoGasto,
    updateTipoGasto,
} from "../../api/gastos/tipo-gastos/TipoGastosAPI";
...
export default function TipoGastosView() {
    ...
    const { mutate: guardar, isPending } = useMutation({
        mutationFn: editando
            ? (nuevoNombre: string) => updateTipoGasto({ id: editando.id, nombre: nuevoNombre })
            : createTipoGasto,
        onError: (error) => {
    ...
    if (isError) {
        return (
            <p className="p-10 text-center text-red-500 font-bold">
                Error al cargar la liquidacion. Verifica tu conexion.
            </p>
        );
    }
                    ...
                    <div className="sm:flex sm:items-center">
                        <div className="sm:flex-auto">
                            ...
                            <p className=" mt-2 text-sm text-gray-700 dark:text-gray-400">
                                Catalogo de tipos de gasto disponibles para las liquidaciones.
                            </p>
                        </div>
                    </div>
                    <div className="mt-3 rounded-lg border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800 dark:border-amber-900/50 dark:bg-amber-950/40 dark:text-amber-200">
                        Usa Activo/Inactivo como flujo normal. La eliminacion fisica queda reservada para casos administrativos.
                    </div>
